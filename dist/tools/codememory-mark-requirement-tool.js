...
export async function createCodeMemoryMarkRequirementTool(conversationStore, getCurrentSessionId, memoryStore) {
    const tool = new CodeMemoryMarkRequirementTool(conversationStore, getCurrentSessionId, memoryStore);
    return {
        name: "codememory_mark_requirement",
        description: "Mark a durable task or hard constraint so future turns can recall the current goal and must-not-break rules. Invoked via the codememory-mark-task / codememory-mark-constraint Skills.",
        params: {
            type: "object",
            properties: {
                kind: {
...
//# sourceMappingURL=codememory-mark-requirement-tool.js.map