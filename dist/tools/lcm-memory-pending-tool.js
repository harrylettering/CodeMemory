/**
 * lcm_memory_pending - debug/admin lifecycle pending-update management.
 *
 ...
 */
export class LcmMemoryPendingTool {
    memoryStore;
    ...
    async apply(params) {
        ...
        const result = await this.memoryStore.applyPendingUpdate({
            ...
            metadata: { appliedByTool: "lcm_memory_pending" },
        });
        return {
            ok: result.ok,
            action: "apply",
...
export async function createLcmMemoryPendingTool(memoryStore) {
    const tool = new LcmMemoryPendingTool(memoryStore);
    return {
        name: "lcm_memory_pending",
...
//# sourceMappingURL=lcm-memory-pending-tool.js.map