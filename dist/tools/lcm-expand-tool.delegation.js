...
export class LcmExpansionDelegation {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    async delegate(params) {
        try {
            ...
            const prompt = params.queryLanguage
                ...
                : params.taskSummary;
            const response = await spawnClaudePrint(prompt, SUBAGENT_SYSTEM_PROMPT, params.timeoutMs ?? this.deps.config.delegationTimeoutMs ?? 120_000);
            if (!response) {
...
function spawnClaudePrint(prompt, systemPrompt, timeoutMs) {
    return new Promise((resolve, reject) => {
        ...
        // client path.
        const child = spawn("claude", ["--bare", "--print", "--output-format", "text", "--append-system-prompt", systemPrompt], { stdio: ["pipe", "pipe", "pipe"] });
        const stdoutChunks = [];
        const stderrChunks = [];
        let settled = false;
