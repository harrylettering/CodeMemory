 ...
 *
 * Exactly matches CodeMemory's `lcm_expand_query` tool.
 */
import { LcmExpansionDelegation } from "./lcm-expand-tool.delegation.js";
import { RetrievalEngine } from "../retrieval.js";
import { LcmContextAssembler } from "../assembler.js";
/**
...
export class LcmExpandQueryTool {
    conversationStore;
    summaryStore;
    deps;
    retrieval;
    ...
    constructor(conversationStore, summaryStore, deps) {
        ...
        this.delegation = new LcmExpansionDelegation(deps);
        this.assembler = new LcmContextAssembler(conversationStore, summaryStore);
    }
     ...
     * Delegates selection + budget-aware packing to LcmContextAssembler so
    ...
    async answerQueryLocally(params, context) {
        ...
        try {
            ...
            this.deps.log.warn(`lcm_expand_query model call failed, falling back to raw snippets: ${err}`);
 ...
 */
export async function createLcmExpandQueryTool(conversationStore, summaryStore, deps) {
    const tool = new LcmExpandQueryTool(conversationStore, summaryStore, deps);
    return {
