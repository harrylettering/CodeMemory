/**
 * lcm_mark_decision — durable decision punctuation.
 *
 * Exposed to the model via the `lcm-mark-decision` Skill, whose body curls
 ...
 */
export class LcmMarkDecisionTool {
    conversationStore;
    getCurrentSessionId;
    memoryStore;
...
export async function createLcmMarkDecisionTool(conversationStore, getCurrentSessionId, memoryStore) {
    const tool = new LcmMarkDecisionTool(conversationStore, getCurrentSessionId, memoryStore);
    return {
        name: "lcm_mark_decision",
        description: "Mark a design or implementation decision so it survives compaction and can be recalled in future sessions. Invoked via the lcm-mark-decision Skill (curl → daemon). Do NOT call for trivial choices.",
        params: {
            type: "object",
            properties: {
                decision: {
...
//# sourceMappingURL=lcm-mark-decision-tool.js.map