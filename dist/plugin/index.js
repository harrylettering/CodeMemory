    ...
    return {
        ...
        async activate(deps) {
            ...
            const getCurrentSessionId = () => {
                const sid = deps?.sessionId ?? deps?.session_id;
                return typeof sid === "string" && sid.length > 0 ? sid : undefined;
            };
            const tools = [
                await createCodeMemoryCheckPriorFailuresTool(memoryStore),
                await createCodeMemoryMarkDecisionTool(engine.getConversationStore(), getCurrentSessionId, memoryStore),
                await createCodeMemoryMarkRequirementTool(engine.getConversationStore(), getCurrentSessionId, memoryStore),
