...
export async function runCodeMemoryMigrations(db) {
  ...
  `);
    ...
    await addColumnIfMissing(db, "conversation_messages", "tier", "TEXT NOT NULL DEFAULT 'S'");
    await addColumnIfMissing(db, "conversation_messages", "tags", "TEXT" // JSON-encoded string array; null when no tags
    );
...
async function ensureMemoryNodeKindSupport(db) {
    const table = await db.get(`SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'memory_nodes'`);
    ...
    try {
        ...
        const hasSourceToolUseIdColumn = await columnExists(db, "memory_nodes", "sourceToolUseId");
        await db.exec(`
