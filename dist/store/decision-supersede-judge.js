...
export class ClaudeDecisionSupersedeJudge {
    config;
    runCompletion;
    constructor(config, runCompletion = (prompt) => runClaudeJudge(prompt, this.config)) {
        this.config = config;
...
function parseJudgeOutput(output, candidates) {
    ...
    for (const entry of raw) {
        ...
        if (!nodeId || !valid.has(nodeId) || seen.has(nodeId))
            continue;
        const verdict = entry.verdict === "SUPERSEDED_BY_NEW" ? "SUPERSEDED_BY_NEW" : "KEEP";
...
//# sourceMappingURL=decision-supersede-judge.js.map