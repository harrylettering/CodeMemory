 ...
 */
export async function loadExploredTargets(db, conversationId, windowMs, nowMs = Date.now()) {
    const cutoff = nowMs - windowMs;
    const rows = await db.all(`SELECT target, lastSeenAt FROM explored_targets
     WHERE conversationId = ? AND lastSeenAt >= ?`, conversationId, cutoff);
    const map = new Map();
    for (const r of rows) {
 ...
 */
export async function flushExploredTargets(db, conversationId, state) {
    if (state._dirtyTargets.size === 0)
        return;
    ...
    try {
        for (const target of dirtyList) {
            ...
            if (lastSeen === undefined)
                continue;
            await db.run(`INSERT INTO explored_targets (conversationId, target, lastSeenAt)
         VALUES (?, ?, ?)
         ON CONFLICT(conversationId, target)
         DO UPDATE SET lastSeenAt = excluded.lastSeenAt`, conversationId, target, lastSeen);
        }
 ...
 */
export async function pruneExploredTargets(db, conversationId, windowMs, nowMs = Date.now()) {
    const cutoff = nowMs - windowMs;
    const result = await db.run(`DELETE FROM explored_targets
     WHERE conversationId = ? AND lastSeenAt < ?`, conversationId, cutoff);
    return result?.changes ?? 0;
}
//# sourceMappingURL=explored-targets-store.js.map