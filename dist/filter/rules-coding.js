...
function hasStructureSignal(text) {
    ...
    if (indentedCode && indentedCode.length >= 2)
        return true;
    // Two or more paragraph breaks (blank line) → multi-section reasoning
    const paragraphBreaks = text.match(/\n\s*\n/g);
    if (paragraphBreaks && paragraphBreaks.length >= 2)
        return true;
    return false;
}
...
function stripRenderedMemory(text) {
...
// Public entry
// ──────────────────────────────────────────────────────────────────────────
export function applyCodingRules(msg, state, ctx) {
...
// User
// ──────────────────────────────────────────────────────────────────────────
function scoreUserMessage(msg, state) {
...
// Assistant
// ──────────────────────────────────────────────────────────────────────────
function scoreAssistantMessage(msg, state, ctx) {
    const parts = msg.metadata?.parts ?? [];
    if (parts.length === 0) {
    ...
    for (const part of parts) {
        if (part.type === "text") {
            ...
            const isLongForm = text.length >= ASSISTANT_TEXT_S_THRESHOLD && hasStructureSignal(text);
        ...
        if (part.type === "tool_use") {
            ...
            if (toolName === "Skill") {
                ...
                if (markRule) {
                    partTier = "S";
                    for (const tag of markRule.tags)
                        tags.add(tag);
                    const skillArgs = typeof input?.args === "string" ? input.args.trim() : "";
                    contentSegments.push(skillArgs ? `${markRule.prefix} ${skillArgs}` : markRule.prefix);
            ...
            if (SCAFFOLDING_TOOLS.has(toolName)) {
                ...
                const withinWindow = lastSeen !== undefined &&
                ...
                if (target && withinWindow) {
                    ...
                    if (target)
                        touchExploredTarget(state, target, ctx.nowMs);
                    partTier = "L";
...
// ──────────────────────────────────────────────────────────────────────────
function scoreToolResults(results, flattened, state) {
    ...
    const readOnlyOrigin = originToolName !== null && EXPLORATION_TOOLS.has(originToolName);
...
function summarizeExplorationResult(toolName, results) {
    let totalBytes = 0;
    let totalLines = 0;
    for (const r of results) {
        const text = typeof r.content === "string" ? r.content : JSON.stringify(r.content ?? "");
...
// Helpers
// ──────────────────────────────────────────────────────────────────────────
function exploreTarget(toolName, input) {
...
function renderMutation(toolName, input) {
    const target = input?.file_path ?? input?.path ?? "?";
    return `[${toolName}] ${target}`;
}
function renderExec(toolName, input) {
