...
- **回滚方案**: 不适用
- **完成时间**: 2026-09-21
- **交付物**: 迁移 35, `scripts/analyze-failure-recall.mjs` 第 ⑫ 节, 两份埋点文档
- **新旧列并存已验证**: `subagent` 保留并与 id 一起写，所以按任一列统计**新行结果一致**；
  只有历史行不同——它们有布尔没有 id。线上实测:

```
子 agent 消息   0 条带 agent id / 83 条标记为 subagent
  差值          83 条是升级前写的，只有布尔标记
```

  **没有重复计数**，也没有把历史行弄丢
- **三个迁移按序跑通**: 真实库副本 7973 条消息、337 个节点完好，三张表的两列全部就位
