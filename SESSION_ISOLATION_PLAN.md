...
- **回滚方案**: 不适用
- **完成时间**: 2026-09-19
- **交付物**: 6 个既有测试文件 + 2 个新文件，共 +428/-35
- **删改审计**: 全分支只删了 4 行断言，都是已记录的两处契约变更：
  `injects across sessions` 被反转；`prefers current conversation` 的 `>= 2` 改 `>= 1`
  **并新增了一条 `not.toContain`**，断言数是增加的，不是放宽的
- **变异检验**（比"有没有删断言"有用得多）:

| 变异 | 结果 |
|---|---|
| 失败召回恒返回空 | 20 条红 ✓ |
| 去掉 `findFailuresByAnchors` 的过滤 | 11 条红 ✓ |
| 去掉 `searchByPlan` 标签查询的过滤 | 1 条红 ✓ |
| 去掉工具层守卫 | 1 条红 ✓ |
| 去掉 `searchByPlan` 的 null 守卫 | **全绿** |
| 去掉关系缝合的 null 守卫 | **全绿** |

- **后两条不是缺口，是防御纵深。** 查明原因：`node:sqlite` 把 `undefined` 归一成 `NULL`，
  而 `n.conversationId = NULL` 恒假，所以 SQL 谓词本身就覆盖了"会话缺失"这一格。
  守卫和谓词覆盖的是**不同的格**（缺失 / 存在但属于别人），只在缺失那一格重叠。
  已把这个分析写进代码注释，避免后人看到"删了也不红"就当冗余删掉
...
- **完成时间**: 2026-09-19
- **交付物**: `CLAUDE.md`, 验收输出
- **验收方式**: 历史埋点行是改造前写的，拿它算只会永远报失败。改成在真实库副本上
  **重放三条召回路径**，对所有会话 × 四个典型提示：

```
节点召回   304 个，跨会话 0
关系缝合   157 条边，端点跨会话 0
失败召回   52 个锚点，以本会话查命中 150 个，以他会话查命中别人的 0 个
```

- **失败那一行特意做了两个方向。** 第一版只统计"跨会话命中 0"，但那是 0/0，
  什么都证明不了。补上"以本会话查命中 150"才说明过滤没有把功能一起关掉
- **CLAUDE.md 的不变量已替换**：原文写着"cross-session retrieval intentionally keeps
  both visible"，与新行为直接矛盾。不改的话下一个人会照着它把隔离"修回去"
