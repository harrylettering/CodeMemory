...
export interface ResolveSucceededAttemptInput {
  conversationId: number;
  fixAttemptNodeId: string;
  files: string[];
  commands: string[];
...
export interface ReopenFailureInput {
  nodeId?: string;
  conversationId?: number;
  files?: string[];
  commands?: string[];
...
export class LifecycleResolver {
  ...
  async resolveFailuresForSucceededAttempt(
    input: ResolveSucceededAttemptInput
  ): Promise<LifecycleResolutionResult> {
  ...
  async reopenFailure(input: ReopenFailureInput): Promise<LifecycleResolutionResult> {
  ...
  async reopenResolvedFailuresForFixAttempt(input: {
    ...
    const relations = await this.memoryStore.getRelationsForNode(
      input.fixAttemptNodeId,
      "from"
    );
    const resolvedFailures: MemoryNodeRecord[] = [];
    for (const relation of relations) {
      if (relation.relationType !== "resolves") continue;
  ...
  async resolveNode(input: ResolveNodeInput): Promise<LifecycleResolutionResult> {
    const node = await this.memoryStore.getNode(input.nodeId);
    if (!node) {
      return {
        action: "none",
  ...
  async supersedeNode(input: SupersedeNodeInput): Promise<LifecycleResolutionResult> {
  ...
  async markNodeStale(input: MarkStaleInput): Promise<LifecycleResolutionResult> {
    return this.markStale(input, "stale_node", "stale_node");
  }

  async markSummaryStale(input: MarkStaleInput): Promise<LifecycleResolutionResult> {
  ...
  private async applyFailureReopen(
    ...
    const fromStatus = node.status as MemoryNodeStatus;
    await this.memoryStore.updateNodeStatus({
      nodeId: node.nodeId,
      toStatus: "active",
      eventType: "reopen_failure",
...
function chooseStrongFailureTarget(
  candidates: FailureAnchorCandidate[]
): (FailureAnchorCandidate & { confidence: number }) | null {
  if (candidates.length === 0) return null;
  const [top, second] = candidates;
  const hasStrongScore = top.score >= 3.5;
  const isClearWinner = !second || top.score - second.score >= 1.25;
  if (!hasStrongScore || !isClearWinner) return null;
  return {
    ...top,
...
function estimateAmbiguousConfidence(candidates: FailureAnchorCandidate[]): number {
  if (candidates.length === 0) return 0;
  const top = candidates[0];
  const second = candidates[1];
  if (!second) return Math.min(0.75, top.score / 10);
