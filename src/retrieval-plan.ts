...
let cachedWorkspaceKey: { root: string; key: string } | null = null;
function workspaceKey(): string {
  const root = resolveWorkspaceRoot();
  if (cachedWorkspaceKey?.root === root) return cachedWorkspaceKey.key;
  const key = createHash("sha256").update(root).digest("hex").slice(0, 8);
  cachedWorkspaceKey = { root, key };
  return key;
}

export function qualifyFileTag(path: string): string {
...
export interface RetrievalPlan {
  ...
  scope: {
    preferCurrentConversation: boolean;
    allowCrossSessionFailures: boolean;
  };
...
const TOPIC_ALIASES: Array<{ topic: string; patterns: RegExp[] }> = [
 ...
 */
export function extractPromptPivots(prompt: string): PromptPivots {
  const text = (prompt || "").slice(0, 4000);

  const filePaths = unique(
    Array.from(
      text.matchAll(/(?:[\w./-]+\/)?[\w-]+\.[a-zA-Z]{1,8}\b/g),
      (m) => m[0]
    ).filter((p) => p.includes("."))
  ).slice(0, 8);
  ...
  const symbols = unique(
    Array.from(
      text.matchAll(/\b([A-Z][a-zA-Z0-9]{2,}|[a-z]+(?:_[a-z0-9]+){1,})\b/g),
      (m) => m[1]
    )
  )
  ...
  const chineseKeywords = unique(
    Array.from(text.matchAll(/[\u4e00-\u9fff]{2,8}/g), (m) => m[0])
...
export function createFastRetrievalPlan(prompt: string): RetrievalPlan {
  ...
  const hasFailureSignal = /失败|报错|错误|踩坑|失败过|broken|fail|failed|error|exception|regression/i.test(
    rawPrompt
  );
  const hasDecisionSignal = /决策|决定|当时|为什么|原因|取舍|rationale|decision|decide|decided/i.test(
    rawPrompt
  );
  const hasModifySignal = /修改|实现|修复|改|更新|edit|modify|fix|change|update/i.test(rawPrompt);
  const hasTestSignal = /测试|跑|重新跑|npm test|pnpm test|yarn test|pytest|vitest|jest/i.test(
    rawPrompt
  );

  const intent: RetrievalIntent =
    hasFailureSignal && (hasModifySignal || hasTestSignal || pivots.filePaths.length > 0)
      ? "modify_and_avoid_prior_failure"
      : hasDecisionSignal
        ? "recall_decision_rationale"
        : hasFailureSignal
  ...
  const riskLevel: RetrievalRiskLevel =
    hasFailureSignal || (hasModifySignal && (hasTestSignal || pivots.filePaths.length > 0))
      ? "high"
      : hasDecisionSignal
        ? "medium"
...
export function inferTopics(text: string, _pivots: PromptPivots = extractPromptPivots(text)): string[] {
  const topics: string[] = [];
...
function buildTagQueries(input: {
  ...
  topics: string[];
  wantedKinds: WantedMemoryKind[];
}): RetrievalPlan["tagQueries"] {
  const tags: RetrievalPlan["tagQueries"] = [];
  for (const file of input.files) tags.push({ tagType: "file", tagValue: qualifyFileTag(file), weight: 2.2 });
  ...
  for (const symbol of input.symbols) tags.push({ tagType: "symbol", tagValue: symbol, weight: 1.4 });
  for (const pkg of input.packages) tags.push({ tagType: "package", tagValue: pkg, weight: 1.2 });
  for (const topic of input.topics) tags.push({ tagType: "topic", tagValue: topic, weight: 1.0 });
  for (const kind of input.wantedKinds) tags.push({ tagType: "kind", tagValue: kind, weight: 0.8 });
...
function inferPackages(text: string, pivots: PromptPivots): string[] {
...
function semanticCommandPrefix(tokens: string[]): string {
...
function segmentChinesePhrase(phrase: string): string[] {
  ...
  for (const word of known) {
    if (phrase.includes(word)) hits.add(word);
  }
  if (hits.size === 0 && phrase.length <= 6) hits.add(phrase);
