...
export interface AgentCorrelationOptions {
  /** How long an entry stays useful. A mark follows its tool call closely. */
  ttlMs?: number;
  /** Hard ceiling, so a tool-heavy session cannot grow this without bound. */
  maxEntries?: number;
  now?: () => number;
}
...
export class AgentCorrelationTable {
  private readonly entries = new Map<string, { identity: AgentIdentity; at: number }>();
  private readonly ttlMs: number;
  private readonly maxEntries: number;
  private readonly now: () => number;

  constructor(options: AgentCorrelationOptions = {}) {
    this.ttlMs = options.ttlMs ?? DEFAULT_TTL_MS;
    this.maxEntries = options.maxEntries ?? DEFAULT_MAX_ENTRIES;
    this.now = options.now ?? (() => Date.now());
  }

  record(toolUseId: string, identity: AgentIdentity): void {
    if (!toolUseId || !identity?.agentId) return;
  ...
  lookup(toolUseId: string | undefined | null): AgentIdentity | undefined {
