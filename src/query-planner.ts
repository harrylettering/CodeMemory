...
import type { MemorySearchCandidate } from "./store/memory-store.js";

export interface QueryPlannerDecision {
  shouldPlan: boolean;
  reason: string;
}

export interface QueryPlannerMetadata {
  source: "fast" | "smart" | "fallback";
  attempted: boolean;
  reason?: string;
  error?: string;
}
...
export interface QueryPlanner {
  plan(input: {
    prompt: string;
    fastPlan: RetrievalPlan;
    reason: string;
  }): Promise<RetrievalPlan>;
}

export class SmartQueryPlanner implements QueryPlanner {
  constructor(
    private config: Pick<
      CodeMemoryConfig,
      "queryPlannerModel" | "queryPlannerTimeoutMs" | "queryPlannerMaxTokens"
    >,
    private runCompletion: (prompt: string) => Promise<string> = (prompt) =>
      runClaudePlanner(prompt, this.config)
  ) {}
...
export function decideSmartPlanning(input: {
  ...
  const hasStrongAnchor =
    input.fastPlan.entities.files.length > 0 ||
  ...
  const asksHistory = /之前|当时|历史|决策|决定|为什么|踩坑|失败过|冲突|原因|rationale|decision|decided|history|previous|prior/i.test(
    input.prompt
  );
...
export function createSmartQueryPlanner(config: CodeMemoryConfig): SmartQueryPlanner {
...
function parsePlannerJson(output: string): any {
...
function coercePlannerOutput(raw: any, fastPlan: RetrievalPlan): RetrievalPlan {
  ...
  const plan: RetrievalPlan = {
    ...
    recallPolicy: {
      ...
      expandSummaries:
        typeof raw.recallPolicy?.expandSummaries === "boolean"
...
function normalizeIntent(value: unknown, fallback: RetrievalPlan["intent"]): RetrievalPlan["intent"] {
  const allowed: RetrievalPlan["intent"][] = [
    "modify_and_avoid_prior_failure",
    "recall_decision_rationale",
    "debug_prior_failure",
    "general_context_lookup",
  ];
  return allowed.includes(value as any) ? (value as RetrievalPlan["intent"]) : fallback;
}

function normalizeRisk(value: unknown, fallback: RetrievalPlan["riskLevel"]): RetrievalPlan["riskLevel"] {
  return value === "low" || value === "medium" || value === "high" ? value : fallback;
}

function normalizeWantedKinds(value: unknown, fallback: WantedMemoryKind[]): WantedMemoryKind[] {
  const allowed = new Set<WantedMemoryKind>([
    "task",
    "constraint",
    "failure",
    "decision",
  ...
  const raw = Array.isArray(value)
    ...
    : [];
  const kinds = raw.filter((kind): kind is WantedMemoryKind => allowed.has(kind));
  return mergeStrings(fallback, kinds) as WantedMemoryKind[];
}

function normalizeTagQueries(raw: unknown, wantedKinds: WantedMemoryKind[]): RetrievalPlan["tagQueries"] {
...
function mergeTagQueries(
  ...groups: RetrievalPlan["tagQueries"][]
): RetrievalPlan["tagQueries"] {
  const merged = new Map<string, RetrievalPlan["tagQueries"][number]>();
  for (const group of groups) {
    for (const tag of group) {
      const tagValue = String(tag.tagValue || "").trim();
      if (!tag.tagType || !tagValue) continue;
