...
export interface MemoryRetrievalResult {
  nodes: MemorySearchCandidate[];
  stitchedRelations: StitchedMemoryRelation[];
  stitchedChains: StitchedMemoryChain[];
  markdown: string;
  estimatedTokens: number;
  summaryEvidence: Map<string, SummaryEvidence>;
  stats: MemoryRetrievalStats;
}

export interface StitchedMemoryRelation {
  relationType: MemoryRelationRecord["relationType"];
  confidence: number;
  score: number;
...
export interface StitchedMemoryChainEdge {
  relationType: MemoryRelationRecord["relationType"];
  confidence: number;
  fromNodeId: string;
  toNodeId: string;
...
export interface StitchedMemoryChain {
  score: number;
  confidence: number;
  nodes: MemoryNodeRecord[];
  edges: StitchedMemoryChainEdge[];
  anchorNodeIds: string[];
}

export interface SummaryEvidenceSnippet {
  summaryId: string;
  kind: "leaf" | "condensed";
  content: string;
}
...
export interface SummaryEvidence {
  ...
  rootKind: "leaf" | "condensed";
  children: SummaryEvidenceSnippet[];
...
export interface MemoryRetrievalStats {
  ...
  stitchedRelationCount: number;
  stitchedChainCount: number;
  summaryEvidenceCount: number;
  relationQueryBatches: number;
  firstHopNodeCount: number;
  firstHopRelationCount: number;
  secondHopNodeCount: number;
  secondHopRelationCount: number;
  estimatedTokens: number;
}

interface RelationStitchBuildResult {
  relations: StitchedMemoryRelation[];
  stats: {
    relationQueryBatches: number;
    queriedNodeCount: number;
    fetchedRelationCount: number;
  };
}

interface ChainBuildResult {
  chains: StitchedMemoryChain[];
  stats: {
    relationQueryBatches: number;
    queriedNodeCount: number;
    fetchedRelationCount: number;
...
export class MemoryRetrievalEngine {
  ...
  async retrieve(input: MemoryRetrievalInput): Promise<MemoryRetrievalResult> {
    ...
    const selected = selectWithinBudget(
      candidates,
      input.plan.recallPolicy.maxInjectedItems,
      input.plan.recallPolicy.tokenBudget
    );
    const selectedTokens = selected.reduce(
      (sum, item) => sum + estimateTokens(item.node.content),
      0
    );
    const stitchedRelationResult = await this.buildRelationStitch(
      selected,
      input.plan,
      input.conversationId,
      Math.max(0, input.plan.recallPolicy.tokenBudget - selectedTokens)
    );
    const stitchedRelations = stitchedRelationResult.relations;
    const stitchedChainResult = await this.buildStitchedChains(
      selected,
      stitchedRelations,
      input.plan,
      input.conversationId,
    ...
    const stitchedTokens = stitchedChains.reduce(
      (sum, chain) => sum + estimateStitchedChainTokens(chain),
      0
    );
    const summaryCandidates = selected.concat(
      stitchedChains.flatMap((chain) =>
        chain.nodes
          .filter(
            (node) =>
              !selected.some((candidate) => candidate.node.nodeId === node.nodeId)
          )
          .map((node) => ({
            node,
            score: chain.score,
    ...
    const summaryEvidence = await this.buildSummaryEvidence(
      summaryCandidates,
      input.plan,
      Math.max(0, input.plan.recallPolicy.tokenBudget - selectedTokens - stitchedTokens)
    );
    ...
    const askedFor = new Set(
      selected.filter(wasAskedFor).map((item) => item.node.nodeId)
    );
    await this.memoryStore.markUsed(
      Array.from(
        new Set([
          ...askedFor,
          ...stitchedChains
    ...
    const estimatedTokens =
      selectedTokens +
      stitchedTokens +
      Array.from(summaryEvidence.values()).reduce(
        (sum, evidence) => sum + estimateEvidenceTokens(evidence),
        0
      );
    ...
    return {
      ...
      stats: {
        ...
        relationQueryBatches:
          stitchedRelationResult.stats.relationQueryBatches +
  ...
  private async buildRelationStitch(
    candidates: MemorySearchCandidate[],
    plan: RetrievalPlan,
    conversationId: number | undefined,
    tokenBudget: number
  ): Promise<RelationStitchBuildResult> {
    ...
    const remainingTokens = Math.min(
      tokenBudget,
      Math.max(120, Math.ceil(plan.recallPolicy.tokenBudget * 0.28))
    );
    const candidateMap = new Map(
      candidates.map((candidate) => [candidate.node.nodeId, candidate.node] as const)
    );
    ...
    const nodeMap = new Map(
      (await this.memoryStore.getNodes(
        Array.from(rawRelations.values()).flatMap((item) => [
    ...
    const stitched = Array.from(rawRelations.values())
      .map((item) => {
        ...
        const anchorNodes = Array.from(item.anchorNodeIds)
          ...
          .filter((node): node is MemoryNodeRecord => !!node);
        let intentBonus = 0;
        const keep = anchorNodes.some((anchorNode) => {
          const otherNode = otherNodeForRawRelation(
            item.relation,
            anchorNode.nodeId,
            fromNode,
            toNode
      ...
      .filter((item): item is StitchedMemoryRelation => !!item)
      .sort((a, b) => {
        if (b.score !== a.score) return b.score - a.score;
        return b.confidence - a.confidence;
      });

    const selected: StitchedMemoryRelation[] = [];
    let usedTokens = 0;
    for (const relation of stitched) {
      const needed = estimateStitchedRelationTokens(relation);
      if (selected.length >= 6) break;
      if (usedTokens + needed > remainingTokens && selected.length > 0) continue;
  ...
  private async buildSummaryEvidence(
    candidates: MemorySearchCandidate[],
    plan: RetrievalPlan,
    tokenBudget: number
  ): Promise<Map<string, SummaryEvidence>> {
    const evidence = new Map<string, SummaryEvidence>();
    if (!this.summaryStore || !plan.recallPolicy.expandSummaries) return evidence;
    if (plan.recallPolicy.maxSummaryDepth <= 0 || tokenBudget < 80) return evidence;

    let remainingTokens = Math.min(tokenBudget, Math.ceil(plan.recallPolicy.tokenBudget * 0.35));
    for (const candidate of candidates) {
      if (remainingTokens < 40) break;
      if (candidate.node.kind !== "summary" || !candidate.node.summaryId) continue;

      const built = await buildEvidenceForSummary(
        this.summaryStore,
        candidate.node.summaryId,
        remainingTokens
      );
      if (!built) continue;

      const used = estimateEvidenceTokens(built);
      if (used > remainingTokens && evidence.size > 0) continue;
  ...
  private async buildStitchedChains(
    candidates: MemorySearchCandidate[],
    stitchedRelations: StitchedMemoryRelation[],
    plan: RetrievalPlan,
    conversationId: number | undefined,
    ...
    const relationMapByNode = new Map<string, StitchedMemoryRelation[]>();
    ...
    const middleNodeIds = Array.from(
      new Set(
        stitchedRelations.flatMap((relation) => [
    ...
    const secondHopNodeIds = Array.from(
      new Set(
        Array.from(secondHopGroups.values()).flatMap((relations) =>
          relations.flatMap((relation) => [relation.fromNodeId, relation.toNodeId])
        )
      )
    );
    const nodeMap = new Map(
      (await this.memoryStore.getNodes(secondHopNodeIds))
        .filter((node) => node.status !== "stale")
        .map((node) => [node.nodeId, node] as const)
    );
    for (const relation of stitchedRelations) {
    ...
    const chains = new Map<string, StitchedMemoryChain>();
    for (const candidate of candidates) {
      ...
      for (const firstHop of firstHopRelations) {
        ...
        for (const rawRelation of secondHopRelations) {
          ...
          if (!tailNodeId || tailNodeId === candidate.node.nodeId) continue;
          const tailNode = nodeMap.get(tailNodeId);
          if (!tailNode || tailNode.nodeId === middleNode.nodeId) continue;
          if (
            !shouldKeepRelationForIntent(
              plan.intent,
              middleNode.kind,
          ...
          addChain(chains, {
            score: chainScore(
              candidate.score,
              [firstHop.relationType, rawRelation.relationType],
              [firstHop.confidence, rawRelation.confidence],
              [candidate.node, middleNode, tailNode],
              conversationId,
              plan.intent
            ),
            confidence: Number(
              ((firstHop.confidence + rawRelation.confidence) / 2).toFixed(3)
            ),
            ...
            anchorNodeIds: Array.from(
              new Set([...firstHop.anchorNodeIds, candidate.node.nodeId])
            ),
          });
        }
      }
    ...
    const selectedChains: StitchedMemoryChain[] = [];
    const remainingTokens = Math.min(
      tokenBudget,
      Math.max(140, Math.ceil(tokenBudget * 0.32))
    );
    let usedTokens = 0;
    for (const chain of ordered) {
      const needed = estimateStitchedChainTokens(chain);
      if (selectedChains.length >= 5) break;
      if (usedTokens + needed > remainingTokens && selectedChains.length > 0) continue;
...
export function renderMemoryMarkdown(
  candidates: MemorySearchCandidate[],
  summaryEvidence: Map<string, SummaryEvidence> = new Map(),
  stitchedChains: StitchedMemoryChain[] = []
): string {
  if (candidates.length === 0 && stitchedChains.length === 0) return "";
 ...
 */
function wasAskedFor(candidate: MemorySearchCandidate): boolean {
  ...
  return candidates
    .map((candidate) => {
      ...
      if (!wasAskedFor(candidate)) {
        const short =
          flat.length > UNASKED_SNIPPET_CHARS
...
function renderChainList(
  ...
  return chains
    .map((chain) => {
      const parts: string[] = [renderRelationNodeSnippet(chain.nodes[0], 96)];
      ...
      const evidence = chain.nodes
        ...
        .filter(Boolean)
        .join(" | ");
      return `- (${chain.edges.length}-hop, score ${chain.score}) ${parts.join(" ")}${
...
function renderChainConnector(
  edge: StitchedMemoryChainEdge,
  fromNode: MemoryNodeRecord,
  toNode: MemoryNodeRecord
): string {
...
function renderRelationNodeSnippet(node: MemoryNodeRecord, maxChars: number): string {
  const snippet = node.content.replace(/\s+/g, " ").slice(0, maxChars);
  return `${node.kind}:${snippet}${renderSource(node)}`;
}

function renderSource(node: MemoryNodeRecord): string {
  if (node.summaryId) return ` [summary:${node.summaryId}]`;
  const messageId = node.metadata.messageId;
  if (messageId) return ` [message:${String(messageId)}]`;
  if (node.sourceId) return ` [${node.source}:${node.sourceId}]`;
  return "";
}
...
async function buildEvidenceForSummary(
...
function toEvidenceSnippet(summary: SummaryRecord): SummaryEvidenceSnippet {
  return {
...
function renderSummaryEvidence(evidence?: SummaryEvidence): string {
  if (!evidence) return "";
  ...
  if (evidence.truncated) parts.push("evidence truncated");
  if (parts.length === 0) return "";

  return ` Evidence: ${parts.join("; ")}`;
}

function estimateEvidenceTokens(evidence: SummaryEvidence): number {
  const childrenTokens = evidence.children.reduce(
    (sum, child) => sum + estimateTokens(child.content),
    0
  );
  return childrenTokens + 16;
}

function estimateStitchedChainTokens(chain: StitchedMemoryChain): number {
  const parts: string[] = [renderRelationNodeSnippet(chain.nodes[0], 96)];
  for (let index = 0; index < chain.edges.length; index += 1) {
    parts.push(renderChainConnector(chain.edges[index], chain.nodes[index], chain.nodes[index + 1]));
    parts.push(renderRelationNodeSnippet(chain.nodes[index + 1], 96));
...
function estimateStitchedRelationTokens(relation: StitchedMemoryRelation): number {
...
function relationPriority(relationType: MemoryRelationRecord["relationType"]): number {
  switch (relationType) {
    case "resolves":
      return 1.45;
...
function chainScore(
  anchorScore: number,
  relationTypes: MemoryRelationRecord["relationType"][],
  confidences: number[],
  nodes: MemoryNodeRecord[],
  conversationId: number | undefined,
  ...
  const conversationScore = nodes.reduce((sum, node, index) => {
    if (!conversationId || node.conversationId !== conversationId) return sum;
  ...
  return Number(
    (
      ...
      conversationScore +
      diversityBonus +
      chainIntentBonus(intent, nodes, relationTypes)
    ).toFixed(3)
  );
}

function indexRelation(
  relationMapByNode: Map<string, StitchedMemoryRelation[]>,
  nodeId: string,
  relation: StitchedMemoryRelation
): void {
...
function otherNodeForRelation(
  relation: StitchedMemoryRelation,
  nodeId: string
): MemoryNodeRecord | null {
  if (relation.fromNode.nodeId === nodeId) return relation.toNode;
  if (relation.toNode.nodeId === nodeId) return relation.fromNode;
  return null;
}

function otherNodeId(
  relation: MemoryRelationRecord,
  nodeId: string
): string | null {
  if (relation.fromNodeId === nodeId) return relation.toNodeId;
  if (relation.toNodeId === nodeId) return relation.fromNodeId;
  return null;
}

function otherNodeForRawRelation(
  relation: MemoryRelationRecord,
  nodeId: string,
  fromNode: MemoryNodeRecord,
  toNode: MemoryNodeRecord
): MemoryNodeRecord | null {
  if (relation.fromNodeId === nodeId) return toNode;
  if (relation.toNodeId === nodeId) return fromNode;
  return null;
}

function uniqueRelationCount(
  groups: Map<string, MemoryRelationRecord[]>
): number {
...
function addChain(
  chains: Map<string, StitchedMemoryChain>,
  chain: StitchedMemoryChain
): void {
...
function shouldKeepRelationForIntent(
  intent: RetrievalPlan["intent"],
  anchorKind: MemoryNodeKind,
  relationType: MemoryRelationRecord["relationType"],
  otherKind: MemoryNodeKind
): boolean {
  const allowedKinds = allowedKindsForIntent(intent);
  if (!allowedKinds.has(anchorKind) || !allowedKinds.has(otherKind)) return false;
...
function relatedPairAllowedForIntent(
  intent: RetrievalPlan["intent"],
  leftKind: MemoryNodeKind,
  rightKind: MemoryNodeKind
): boolean {
...
function allowedKindsForIntent(intent: RetrievalPlan["intent"]): Set<MemoryNodeKind> {
  switch (intent) {
    case "modify_and_avoid_prior_failure":
...
function shouldKeepTwoHopChain(
  intent: RetrievalPlan["intent"],
  nodes: MemoryNodeRecord[],
  relationTypes: MemoryRelationRecord["relationType"][]
): boolean {
  ...
  switch (intent) {
    case "modify_and_avoid_prior_failure":
      return (
        (matchesKinds(kinds, ["task", "fix_attempt", "failure"]) &&
...
function relationIntentBonus(
  ...
  if (intent === "modify_and_avoid_prior_failure") {
    if (
      (relationType === "attemptedFixFor" || relationType === "resolves") &&
  ...
  if (intent === "recall_decision_rationale") {
    if (relationType === "supersedes" || relationType === "conflictsWith") return 0.28;
    if (otherKind === "summary" || anchorKind === "summary") return 0.18;
  }
  if (intent === "debug_prior_failure") {
    if (otherKind === "failure" || anchorKind === "failure") return 0.22;
    if (otherKind === "fix_attempt" || anchorKind === "fix_attempt") return 0.18;
  }
  if (intent === "general_context_lookup") {
    if (anchorKind === "task" || anchorKind === "constraint") return 0.12;
  }
  return 0;
}

function chainIntentBonus(
  intent: RetrievalPlan["intent"],
  nodes: MemoryNodeRecord[],
  relationTypes: MemoryRelationRecord["relationType"][]
): number {
  ...
  switch (intent) {
    case "modify_and_avoid_prior_failure":
      ...
      if (matchesKinds(kinds, ["task", "decision", "failure"])) return 0.28;
      if (matchesKinds(kinds, ["task", "decision", "constraint"])) return 0.22;
      break;
    case "recall_decision_rationale":
      ...
      break;
    case "debug_prior_failure":
      if (kindSet.has("failure") && kindSet.has("fix_attempt")) return 0.36;
      if (kindSet.has("failure") && kindSet.has("decision")) return 0.24;
      break;
    case "general_context_lookup":
    default:
      if (
