...
const CODING_ERROR_PATTERNS: RegExp[] = [
  // shell / process
  /exit code\s*[^0]/i,
...
function isLikelyCodingError(raw: string): boolean {
...
// Track mutations we've seen recently; used to link an error back to
// what triggered it. We keep filePath and command as separate fields so
// the resulting NegExp record can be queried by either dimension.
...
export class NegExpExtractor {
  private recentMutations: RecentMutation[] = [];
   ...
   */
  private lastSeenSignatureSeq = new Map<string, number>();
  private static readonly DEDUP_SEQ_WINDOW = 10;

  /**
   * Most recently observed mutation, or null. Used by the daemon to
   * attribute "user said it's fixed" to the right target.
   */
  getMostRecentMutation(): RecentMutation | null {
   ...
   */
  observeMessage(msg: JsonlMessage, seq: number): void {
  ...
  private recordRecentMutation(m: RecentMutation): void {
  ...
  private extractMutationTarget(
    toolName: string,
    input: any
  ): { filePath?: string; command?: string; description: string } | null {
    ...
    const snippet = (s: unknown, max = 160): string => {
